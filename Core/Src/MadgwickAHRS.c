...
#define sampleFreq	998.0f		// sample frequency in Hz
...
volatile static float roll = 0.0f, pitch = 0.0f,  yaw = 0.0f;
//---------------------------------------------------------------------------------------------------
	...
	q3 *= recipNorm;
	  ...
	  if (test > 0.499) {
	    ...
	    yaw = 0;
		  ...
		  roll = atan2(2*QY*QW - 2*QX*QZ, 1 - 2*QY*QY - 2*QZ*QZ)*180/PI;
		  pitch = asin(2*QX*QY + 2*QZ*QW)*180/PI;
		  yaw = atan2(2*QX*QW - 2*QY*QZ, 1 - 2*QX*QX - 2*QZ*QZ)*180/PI;
	  }


//	roll = atan2(2*(q0*q3+q2*q3), 1-2*(q1*q1+q2*q2));
